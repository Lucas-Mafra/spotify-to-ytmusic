...
The naive strategy takes the first search result, which often picks covers,
...
_FROM_RE = re.compile(r"\((?:from|do)\b[^()]*\)|\[(?:from|do)\b[^\[\]]*\]", re.IGNORECASE)
...
def _artist_score(track_artists: tuple[str, ...], candidate_artists: list[str]) -> float:
    ...
    scores = [
        max(similarity(normalize(artist), candidate) for candidate in normalized_candidates)
...
def score_match(
    ...
    artist_score = _artist_score(track.credits, candidate_artists)
